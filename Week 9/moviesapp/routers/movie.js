...
module.exports = {
    ...
    // *DELETE all movies between year1 and year2 - app.delete('/movies/:year1/:year2', movies.deleteByYear);
    deleteByYear: function (req, res) {
        Movie.deleteMany({$and: [{year: {$lte: req.params.year1}}, {year: {$gte: req.params.year2}}]}, function (err) {
            if (err) return res.status(400).json(err);
            res.json({
